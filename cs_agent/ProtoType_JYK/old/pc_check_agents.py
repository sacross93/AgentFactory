...
def table_abstract_agent(user_query):
    ...
    Your response must be a JSON object with three keys:
        ...
        table_names:
            ...
            - Table with name gpu_mb_compatibility does not exist
            - Only include tables that are listed in the provided Tables section
...
def query_modify_agent(user_query, table_info, query_info, result):
    ...
    12. For RAM queries, use the 'capacity' column to filter by GB size (e.g., WHERE capacity >= 16 for 16GB or more)
    13. For RAM compatibility with motherboards, check the memory_standard or memory_type columns
    ...
    query_modify_info = query_modify_chain.invoke({"user_query":user_query, "user_meaning":table_info['user_meaning'], "reasons":table_info['reason'] + query_info['reason'], "previous_query":query_info['query'], "result":result, "tables":tables})
...
def check_result_agent(user_query, result):
    ...
    Here is the output schema:
    ```
    {{"properties":{{"reason": {{"title": "Reason", "description": "Explanation of why the result is suitable or not suitable for addressing the user's requirements", "type": "string"}}, "result": {{"title": "Result", "description": "Whether the data is sufficient to provide useful information", "type": "boolean"}}, "answer": {{"title": "Answer", "description": "The comprehensive answer listing all matching components with their details", "type": "string"}}}}, "required": ["reason", "result", "answer"]}}
    ```
